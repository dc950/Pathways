...
def webcrawler():
        ...
        for link in soup.find_all(href=re.compile("job-profile")):
            ...
            print("\nRELATED SKILLS")
            for link2 in soup2.find_all('div',{'class':'field field-name-field-related-skills field-type-taxonomy-term-reference field-label-above'}):
                skillList=[]
                ...
                print(skillList)
            print("\nRELATED SUBJECTS")
            for link2 in soup2.find_all('div',{'class':'field field-name-field-related-subjects field-type-taxonomy-term-reference field-label-above'}):
                subList=[]
                for counter in range(0, len(link2.contents[3])):
                    listofskills=link2.contents[3].contents[counter].string.split()
                    if listofskills == []:
            ...
            for link2 in soup2.find_all('div',{'class':'field field-name-field-desirable-qualifications field-type-text field-label-above'}):
                qualList=[]
                for counter in range(0, len(link2.contents[3])):
                    listofskills=link2.contents[3].contents[counter].string.split()
                    if listofskills == []:
